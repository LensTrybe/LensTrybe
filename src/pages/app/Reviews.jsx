...
const FILT = [['all', 'All'], ['unanswered', 'Unanswered'], ['5', '5 star'], ['4', '4 and up'], ['low', '3 and under'], ['featured', 'Featured'], ['private', 'Private'], ['imported', 'Added by you'], ['flagged', 'Flagged']]
const SRC = { LensTrybe: ['Verified booking', 'ok'], Google: ['Google', 'grey'], Imported: ['Added by you', 'grey'] }

export default function Reviews() {
  const F = useFlows(); const { s, toast } = F; const rs = s.reviews, rules = s.reviewRules, RQ = s.reviewRequests || []
  ...
  const post = (r, text) => { if (!text?.trim()) return toast('Write a line first, or tap Draft.'); F.upd('reviews', r.id, { reply: text.trim() }); if (r.t2) F.say(r.t2, 'me', text.trim()); setDraft(d => ({ ...d, [r.id]: '' })); toast('Reply posted under ' + r.who.split(' ')[0] + "'s review.") }
  ...
  return (
    <section className="view rvw">
      <div className="vh">
        <div><h1>Reviews</h1><p>Verified where they came through a booking, asked for three days after the gallery opens, answered in your words.</p></div>
        <div className="acts">
      ...
      <div className="grid">
        ...
        <div className="card lg s8">
          ...
          <div className="revs">
            {list.map(r => { const [sl, sc] = SRC[r.src] || [r.src, 'grey']; return <div key={r.id} className={'rev' + (r.st === 'private' ? ' priv' : '') + (r.st === 'flagged' ? ' flg' : '') + (r.featured ? ' feat' : '')}>
              ...
              {r.st === 'flagged' && <p className="note2 warn2"><Icon name="shield" size={12} /> Under review · {r.flag?.why === 'never' ? 'never a client' : r.flag?.why} · hidden from your profile since {nice(r.flag?.at || TODAY)}.</p>}
              <p>{r.t}</p>
              {r.reply ? <div className="rr"><span className="lm" style={{ width: 12, height: 12 }} /><span><b>You</b> {r.reply}</span><button className="lnk" onClick={() => F.replyReview(r.id)}>Edit</button></div>
                : r.st !== 'flagged' && <div className="rc"><input value={draft[r.id] ?? ''} onChange={e => setDraft(d => ({ ...d, [r.id]: e.target.value }))} placeholder={r.st === 'private' ? 'Reply privately, in your words' : 'Reply in your words, or let Lumi draft one'} onKeyDown={e => e.key === 'Enter' && post(r, draft[r.id])} /><button className="act2" onClick={() => setDraft(d => ({ ...d, [r.id]: F.draftReply(r).replace(/\n/g, ' ') }))}>Draft</button><button className="btn w sm" onClick={() => post(r, draft[r.id])}>Post</button></div>}
              <div className="racts">
                {r.st === 'public' && <button className="act2" onClick={() => F.featureReview(r.id)}>{r.featured ? 'Unpin' : 'Feature on site'}</button>}
        ...
        <div className="s4 side">
          ...
          {priv.length ? <div className="tlumi"><span className="lm" /><div>{priv[0].who.split(' ')[0]} left {priv[0].n} stars and it is private for now. The gallery ran long, which is fair. A reply today and a small make-good keeps it off the profile and keeps the client.<div className="acts"><button className="y" onClick={() => F.replyReview(priv[0].id)}>Reply now</button><button onClick={() => F.publishReview(priv[0].id)}>Publish as is</button></div></div></div>
            : private5.length > 1 ? <div className="tlumi"><span className="lm" /><div>{private5.map(r => r.who.split(' ')[0]).join(' and ')} left five stars and have no reply. Two lines each, in your voice, ready to post.<div className="acts"><button className="y" onClick={() => { private5.forEach(r => { const t = F.draftReply(r); F.upd('reviews', r.id, { reply: t }); if (r.t2) F.say(r.t2, 'me', t) }); toast('Replied to ' + private5.length + '.') }}>Post all</button><button onClick={() => toast('Left for you.')}>I will write them</button></div></div></div>
