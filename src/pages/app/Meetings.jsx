...
export default function Meetings() {
  ...
  const reschedule = () => F.open({ title: 'Reschedule · ' + m.who, sub: 'They get the new time by text and email, and both calendars move.', cta: 'Move it', fields: [{ k: 'd', l: 'Date', type: 'date', required: true, half: true, value: m.when.slice(0, 10), min: TODAY }, { k: 'time', l: 'Time', type: 'time', required: true, half: true, value: m.when.slice(11) }, { k: 'how', l: 'How', type: 'select', value: m.how, options: ['Video', 'Phone', 'In person'] }], submit: v => { upd('meetings', m.id, { when: v.d + 'T' + v.time, how: v.how, st: v.d === TODAY ? 'today' : 'up' }); F.say(m.th, 'sys', 'Meeting moved to ' + when(v.d + 'T' + v.time)); toast('Moved. ' + m.who.split(' ')[0] + ' has the new time.') } })
  const cancel = () => F.confirm({ title: 'Cancel with ' + m.who + '?', body: 'They get a message from you. The slot opens up on your link.', cta: 'Cancel meeting', danger: true, onYes: () => { F.del('meetings', m.id); F.say(m.th, 'sys', 'Meeting cancelled · ' + when(m.when)); setSel(null); toast('Cancelled. ' + m.who.split(' ')[0] + ' knows.') } })
  ...
  return (
    <section className="view">
      <div className="vh">
        ...
        <div className="acts"><span className="syncb"><i />Google Calendar · synced</span><button className="btn w" onClick={() => F.newMeeting()}><Icon name="plus" size={15} />New meeting</button></div>
      </div>
      <div className="grid">
        <div className="s12"><div className="kp">
          {[['This week', String(wk), list.filter(x => x.st !== 'done').slice(0, 2).map(x => x.who.split(' ·')[0]).join(', ') || 'nothing booked', ''], ['Booked from your link', '4', 'this month', 'n'], ['Turned into bookings', '3 of 4', 'discovery calls, this quarter', ''], ['No-shows', '0', 'reminders go 24h and 1h before', 'n']].map(([l, v, e, w]) => <div key={l} className="k lg"><small>{l}</small><b>{v}</b><em className={w}>{e}</em></div>)}
        </div></div>
          ...
          <div className="up mt">
            {list.map(x => { const t = TYPES.find(y => y.id === x.t) || TYPES[0], st = stOf(x); return (
              <div key={x.id} className={'d' + (m?.id === x.id ? ' on' : '') + (st === 'today' ? ' now' : '')} onClick={() => setSel(x.id)}>
                ...
                <div><b>{x.who}</b><small>{t.n} · {t.m} min · {x.how} · {when(x.when).split(' · ')[1]}</small></div>
                <span className={'st ' + (st === 'today' ? 'now' : st === 'done' ? 'grey' : 'ok')}>{st === 'today' ? 'Today' : st === 'done' ? 'Done' : 'Booked'}</span>
              </div>) })}
            {!list.length && <div className="tempty">Nothing here. <button className="lnk" onClick={() => F.newMeeting()}>Book one</button></div>}
          </div>

          <div className="h" style={{ marginTop: 22 }}><b>Your booking link</b><span className="lnkrow inl"><Icon name="globe" size={13} /><span>mara.lenstrybe.com/meet</span><button onClick={() => { try { navigator.clipboard?.writeText('https://mara.lenstrybe.com/meet')?.catch(() => {}) } catch {} toast('Link copied') }}>Copy</button></span></div>
        ...
        {m && <div className="s4 side">
          <div className="card lg">
            <div className="h"><b style={{ display: 'flex', alignItems: 'center', gap: 10 }}><span className="av" style={{ background: m.g, width: 30, height: 30, borderRadius: 10 }} />{m.who}</b><span className={'st ' + (stOf(m) === 'today' ? 'now' : m.st === 'done' ? 'grey' : 'ok')}>{stOf(m) === 'today' ? 'Today' : m.st === 'done' ? 'Done' : 'Booked'}</span></div>
            <div className="kv"><span>What</span><b>{T.n} · {T.m} min</b></div>
            <div className="kv"><span>When</span><b>{when(m.when)}</b></div>
            <div className="kv"><span>How</span><b>{m.how}{m.how === 'Video' ? ' · link in the thread' : ''}</b></div>
          ...
          <div className="card lg"><div className="h"><b>{m.st === 'done' ? 'Notes' : 'Before the call'}</b><small className="lumi-by"><span className="lm" style={{ width: 12, height: 12 }} />Lumi</small></div><p className="note2">{m.notes || m.prep || 'Nothing yet.'}</p></div>
          {m.id === 1 && m.st !== 'done' && <div className="tlumi"><span className="lm" /><div>Album selection is at 2:30. I've opened the proof with the two speech spreads they asked about. Want the anniversary session mentioned at the end?<div className="acts"><button className="y" onClick={() => { upd('meetings', 1, { prep: m.prep + ' Raise the first anniversary session at the end.' }); toast('Added to your notes for the call.') }}>Yes, note it</button><button onClick={() => toast('Left out.')}>Not this time</button></div></div></div>}
          {m.id === 2 && m.st !== 'done' && !s.ledger.some(r => r.t === 'ruby' && r.k === 'q') && <div className="tlumi"><span className="lm" /><div>Ruby's quote is drafted but not sent. Sending it before the call means they see the price first; sending after means you set it up. Your call.<div className="acts"><button className="y" onClick={() => F.newDoc('q', { client: 'ruby', d: 'Full day, Maleny', v: 3200 })}>Send now</button><button onClick={() => toast('Held until after the call.')}>After the call</button></div></div></div>}
        </div>}
      </div>
    </section>
  )
