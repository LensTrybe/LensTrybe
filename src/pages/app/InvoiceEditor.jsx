...
// The invoice, on screen, as the client will see it. Everything on the page is editable in place:
...
const money = n => '$' + (Math.round((n || 0) * 100) / 100).toLocaleString('en-AU', { minimumFractionDigits: 2, maximumFractionDigits: 2 })
const Ed = ({ v, set, ph, cls = '', multi, type = 'text', right, w }) => multi
  ? <textarea className={'ed ' + cls} value={v} placeholder={ph} onChange={e => set(e.target.value)} rows={Math.max(1, (v || '').split('\n').length)} />
...
export default function InvoiceEditor() {
  ...
  const blank = () => ({
    no: F.next('inv'), issued: TODAY, client: state?.client || '', for: state?.d || '',
    from: { biz: s.settings.biz, n: s.profile.n, abn: s.settings.abn, em: s.settings.email, ph: s.settings.phone, addr: s.settings.addr || s.profile.city, pay: D.pay || 'BSB 484-799 · Acc 1204 5561 · ' + s.settings.biz },
    to: { n: '', co: '', em: '', ph: '', addr: '' },
    items: state?.items || [{ d: state?.d || '', q: 1, r: state?.v || '' }],
    gstIncl: s.brand.gst !== 0, gstOn: true,
    dep: { on: !!D.depOn, pct: D.depPct ?? 30, amt: '', due: TODAY }, balDue: addDays(TODAY, D.balDays ?? 7),
    notes: D.notes || 'Thank you for your business.', terms: D.terms || s.brand.terms || 'Deposit locks the date. Balance due seven days before the shoot. Card or transfer.',
  })
  const [inv, setInv] = useState(() => existing?.doc ? existing.doc : blank())
  const [dirty, setDirty] = useState(false), [pct, setPct] = useState(true)
  const paper = useRef()
  ...
  const pickClient = cid => { const p = s.people.find(x => x.id === cid); up(x => ({ ...x, client: cid, to: p ? { n: p.n, co: p.co !== p.n ? p.co : '', em: p.em || '', ph: p.ph || '', addr: p.addr || '' } : x.to })) }
  ...
  const client = s.people.find(p => p.id === inv.client)
  const status = existing?.stt || 'Draft'
  // ── save / send ──
  const record = st => ({ k: 'inv', who: inv.to.n || client?.n || 'Client', d: inv.for || items[0]?.d || 'Invoice', date: inv.dep.on ? inv.dep.due : inv.balDue, st: st === 'Sent' ? 'sent' : 'grey', stt: st, v: Math.round(total), t: inv.client || existing?.t, doc: inv, sched: inv.dep.on ? [['Deposit', depAmt, inv.dep.due], ['Balance', bal, inv.balDue]] : [['Balance', total, inv.balDue]] })
  const save = (st = status === 'Draft' ? 'Draft' : status) => { if (existing) F.upd('ledger', existing.id, record(st)); else { F.add('ledger', { id: inv.no, ...record(st) }, 'inv') } setDirty(false); return inv.no }
  const send = () => {
    ...
    if (tid) { F.upd('threads', tid, t => ({ docs: [...t.docs.filter(d => !d[0].includes(no)), ['Invoice #' + no, 'sent', 'Sent']], line: [...t.line.filter(m => !m.doc?.includes(no)), { doc: 'Invoice #' + no + (inv.for ? ' · ' + inv.for : ''), d: money(total) + (inv.dep.on ? ' · deposit ' + money(depAmt) + ' due ' + nice(inv.dep.due) : ' · due ' + nice(inv.balDue)), st: 'sent', stt: 'Sent' }], last: 'Invoice ' + no + ' sent' })); F.upd('threads', tid, t => ({ stage: Math.max(t.stage, 4) })) }
    toast('Invoice ' + no + ' sent to ' + (inv.to.n || client.n) + (inv.to.em ? ' at ' + inv.to.em : '') + '.'); nav('/app/invoicing')
  }
  const saveDefaults = () => { F.patch('settings', { invoice: { depOn: inv.dep.on, depPct: Number(inv.dep.pct) || 30, balDays: Math.max(0, Math.round((new Date(inv.balDue) - new Date(inv.issued)) / 864e5)), notes: inv.notes, terms: inv.terms, pay: inv.from.pay }, biz: inv.from.biz, abn: inv.from.abn, email: inv.from.em, phone: inv.from.ph, addr: inv.from.addr }); toast('Saved as your defaults. Every new invoice starts like this.') }
  const print = () => { const w = window.open('', '_blank'); if (!w) return; w.document.write('<html><head><title>Invoice ' + inv.no + '</title><style>body{font-family:Inter,-apple-system,sans-serif;color:#14111a;padding:40px;max-width:760px;margin:auto}input,textarea{border:0;background:none;font:inherit;color:inherit;padding:0;resize:none;width:100%}.r{text-align:right}table{width:100%;border-collapse:collapse}td,th{padding:8px 0;border-bottom:1px solid #eee;text-align:left}.tot td{font-weight:700}.rm,.addl,.hint{display:none}.chip{display:none}.ptop{display:flex;justify-content:space-between}</style></head><body>' + paper.current.innerHTML + '</body></html>'); w.document.close(); setTimeout(() => w.print(), 300) }
  const del = () => F.confirm({ title: 'Delete ' + inv.no + '?', body: 'It comes out of the ledger and the thread.', cta: 'Delete', danger: true, onYes: () => { if (existing) F.removeDoc(existing.id); nav('/app/invoicing') } })
  const acc = s.brand.accent || '#8DF3D6'

  return (
    <section className="view inved">
      <div className="vh">
        <div><Link to="/app/invoicing" className="lnk back"><Icon name="back" size={13} />Invoicing</Link><h1>{existing ? 'Invoice' : 'New invoice'} <em>{inv.no}</em></h1><p>{status === 'Draft' ? 'Everything on the page is editable. Tap any text to change it.' : status + (existing?.date ? ' · due ' + nice(existing.date) : '')}</p></div>
        <div className="acts">
          {existing && status !== 'Paid' && <button className="btn g" onClick={() => { F.markPaid(existing.id); nav('/app/invoicing') }}><Icon name="check" size={15} />Mark paid</button>}
          <button className="btn g" onClick={print}><Icon name="deliver" size={15} />PDF</button>
          <button className={'btn g' + (dirty ? '' : ' quiet')} onClick={() => { save(); toast('Saved.') }}>{dirty ? 'Save draft' : 'Saved'}</button>
          <button className="btn w" onClick={send}><Icon name="arrow" size={15} />{status === 'Sent' ? 'Send again' : 'Send'}</button>
        </div>
      </div>
      <div className="grid">
        <div className="s8">
          <div className="paper inv" ref={paper} style={{ '--acc': acc, fontFamily: `'${s.brand.body || 'Inter'}', sans-serif` }}>
            <div className="ptop">
              <div className="from">
                <b><span className="mark" style={{ background: acc }} /><Ed v={inv.from.biz} set={v => upFrom('biz', v)} ph="Your business" cls="big" /></b>
              ...
              <div className="meta">
                <h3 style={{ fontFamily: `'${s.brand.head || 'Inter'}', serif` }}>Tax invoice</h3>
                ...
                <div className="mrow"><span>Due</span><Ed v={inv.balDue} set={v => up({ balDue: v })} type="date" right w={140} /></div>
                <span className={'chip ' + (status === 'Paid' ? 'ok' : status === 'Sent' ? 'sent' : 'grey')}>{status}</span>
              </div>
            </div>

            <div className="to">
              <small>Bill to</small>
              <div className="pick"><select value={inv.client} onChange={e => pickClient(e.target.value)}><option value="">Pick from Contacts…</option>{s.people.map(p => <option key={p.id} value={p.id}>{p.n}</option>)}</select></div>
        ...
        <div className="s4 side sticky">
          <div className="card lg"><div className="h"><b>Payment schedule</b></div>
            <div className="brows one">
              <label className="brow"><span>Take a deposit<small>The rest is due before the shoot</small></span><span className={'sw2' + (inv.dep.on ? ' on' : '')} role="switch" aria-checked={inv.dep.on} onClick={() => up(x => ({ ...x, dep: { ...x.dep, on: !x.dep.on } }))}><i /></span></label>
            </div>
            {inv.dep.on && <div className="dep">
              ...
              <div className="sfs">
                ...
                <div className="sf half"><label>Deposit due</label><input type="date" value={inv.dep.due} onChange={e => up(x => ({ ...x, dep: { ...x.dep, due: e.target.value } }))} /></div>
                <div className="sf half"><label>Balance due</label><input type="date" value={inv.balDue} onChange={e => up({ balDue: e.target.value })} /></div>
          ...
          <div className="card lg"><div className="h"><b>Client</b>{client && <Link className="lnk" to={'/app/thread/' + client.id}>Thread <Icon name="arrow" size={12} /></Link>}</div>
            <p className="note2">{client ? client.n + ' · ' + (client.em || 'no email') + '. Filled from Contacts; change anything on the invoice itself.' : 'Pick from Contacts on the invoice, or type the details straight in. '}{!client && <button className="lnk" onClick={() => F.newClient(r => pickClient(r.pid))}>Add a new client</button>}</p>
          </div>
          <div className="tlumi"><span className="lm" /><div>{inv.dep.on ? 'Deposit invoices get paid 2.4× faster than full ones in your history. The reminder for the balance goes out on its own three days before.' : 'A deposit locks the date and gets paid faster. Thirty percent is your usual.'}{!inv.dep.on && <div className="acts"><button className="y" onClick={() => up(x => ({ ...x, dep: { ...x.dep, on: true } }))}>Add a 30% deposit</button></div>}</div></div>
          {existing && <button className="lnk" style={{ opacity: .6 }} onClick={del}>Delete invoice</button>}
        </div>
      </div>
    </section>
  )
