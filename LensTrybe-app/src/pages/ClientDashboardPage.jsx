...
/* A job listing's own state, in the client's words rather than the column's. */
...
export default function ClientDashboardPage() {
  ...
  function deleteThread(t, e) {
    e.stopPropagation()
    confirm({
      title: 'Delete this conversation?',
      body: `Everything you and ${t.nickname ?? getCreativeName(t)} have said to each other goes with it. This cannot be undone.`,
  ...
  const s = {
    ...
    threadWrap: { position: 'relative', marginBottom: 2 },
    thread: (active) => ({ display: 'block', width: '100%', textAlign: 'left', fontFamily: 'inherit', padding: '10px 12px', cursor: 'pointer', borderRadius: 10, background: active ? 'rgba(29,185,84,0.12)' : 'transparent', border: active ? '1px solid rgba(29,185,84,0.35)' : '1px solid transparent' }),
    threadName: { fontSize: '13px', fontWeight: 600, color: 'var(--lt-text)', marginBottom: '2px' },
    threadPreview: { fontSize: '12px', color: 'var(--lt-muted)' },
    delBtn: { position: 'absolute', top: '6px', right: '6px', opacity: 0, background: 'none', border: 'none', color: 'var(--lt-pink-text)', cursor: 'pointer', fontSize: '14px', padding: '2px 6px', transition: 'opacity 0.2s' },
      ...
      <div style={s.body} className="cd-body">
            ...
            <>
              ...
              <div className="cd-threads" style={s.threadList}>
                {threads.map(t => (
                  ...
                  >
                    <button type="button" style={s.thread(selected?.id === t.id)} onClick={() => setSelected(t)}>
                    ...
                    <button
                      ...
                      className="del"
                      style={s.delBtn}
                      aria-label={`Delete the conversation with ${t.nickname ?? getCreativeName(t)}`}
        ...
        <div style={s.main}>
                ...
                <div style={s.chatHeader}>
                  {editingNickname ? (
                    ...
                    <>
                      ...
                      <button type="button" style={s.editIcon} aria-label="Rename this conversation" onClick={() => setEditingNickname(true)}>Rename</button>
                    </>
                  )}
                </div>
                ...
                <div style={s.replyBar}>
                  <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 6, minWidth: 0 }}>
                    ...
                    {replyModerationError ? (
                      <div style={{ fontSize: '12px', color: 'var(--lt-pink-text)', fontFamily: 'var(--font-ui)' }}>{replyModerationError}</div>
                    ) : null}
                  </div>
          ...
          {view === 'creatives' && (
            <ClientCreativesView
              userId={user?.id}
              threadCreatives={creatives}
              onMessage={goToCreativeThread}
              onFindCreative={() => navigate('/creatives')}
            />
          )}
            ...
            <div style={s.content}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', maxWidth: '640px' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', padding: '24px', borderRadius: '18px', background: 'var(--lt-surface)', border: '1px solid rgba(255,45,120,0.35)' }}>
                  <div style={{ fontSize: '15px', fontWeight: 800, color: 'var(--lt-pink-text)' }}>Delete account</div>
          ...
          {view === 'bookings' && (
            <ClientBookingsView
              ...
              onMessageCreative={goToCreativeThread}
            />
          )}
              ...
              {jobs.length === 0 ? (
                ...
                <div key={job.id} style={{ background: 'var(--lt-surface)', border: '1px solid var(--lt-border)', borderRadius: '12px', marginBottom: '12px', overflow: 'hidden' }}>
                  <button
                    ...
                    style={{ width: '100%', textAlign: 'left', fontFamily: 'inherit', background: 'none', border: 'none', padding: '16px 20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '12px', cursor: 'pointer' }}
                    onClick={() => setExpandedJob(expandedJob === job.id ? null : job.id)}
                  >
                    <div style={{ minWidth: 0 }}>
                      <div style={{ fontSize: '15px', fontWeight: 600, color: 'var(--lt-text)' }}>{job.title}</div>
                      <div style={{ fontSize: '12px', color: 'var(--lt-muted)', marginTop: '4px' }}>{[job.location, job.budget_range, jobState(job)].filter(Boolean).join(' · ')}</div>
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '12px', flexShrink: 0 }}>
                      <span style={{ padding: '4px 12px', background: 'rgba(29,185,84,0.12)', border: '1px solid rgba(29,185,84,0.3)', borderRadius: '999px', fontSize: '12px', fontWeight: 700, color: 'var(--lt-green-text)', whiteSpace: 'nowrap' }}>
                        {job.job_applications?.length ?? 0} application{job.job_applications?.length !== 1 ? 's' : ''}
                      </span>
                      <span aria-hidden style={{ color: 'var(--lt-muted)', fontSize: '16px' }}>{expandedJob === job.id ? '▲' : '▼'}</span>
                    </div>
                  </button>
                        ...
                        job.job_applications.map(app => (
                          <div key={app.id} style={{ padding: '16px', background: 'var(--lt-surface-2)', borderRadius: '10px', marginBottom: '10px', border: '1px solid var(--lt-border)' }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '10px' }}>
                              ...
                              <div style={{ fontSize: '18px', fontWeight: 800, color: 'var(--lt-green-text)', flexShrink: 0 }}>{aud(app.price)}</div>
                            </div>
                            {app.includes && (
                            ...
                            {isApplicationPending(app) && (
                              <div style={{ display: 'flex', gap: '8px', marginTop: '12px', paddingTop: '12px', borderTop: '1px solid var(--lt-hairline)' }}>
                                <button
                                  ...
                                  style={{ padding: '8px 20px', background: '#1DB954', border: 'none', borderRadius: '8px', color: '#04120a', fontSize: '13px', fontWeight: 700, cursor: 'pointer', fontFamily: 'var(--font-ui)' }}
                                >
                                  ✓ Accept
                                </button>
                                <button
                                  ...
                                  style={{ padding: '8px 20px', background: 'rgba(255,45,120,0.10)', border: '1px solid rgba(255,45,120,0.30)', borderRadius: '8px', color: 'var(--lt-pink-text)', fontSize: '13px', fontWeight: 600, cursor: 'pointer', fontFamily: 'var(--font-ui)' }}
                                >
                                  Decline
                                </button>
                              </div>
                            ...
                            {app.status === 'accepted' && (
                              <div style={{ marginTop: '12px', padding: '8px 12px', background: 'rgba(29,185,84,0.1)', border: '1px solid rgba(29,185,84,0.2)', borderRadius: '8px', fontSize: '12px', fontWeight: 700, color: 'var(--lt-green-text)' }}>
                                ✓ Accepted, message thread created
                              </div>
                            )}
                            {app.status === 'declined' && (
                              <div style={{ marginTop: '12px', padding: '8px 12px', background: 'rgba(255,45,120,0.08)', border: '1px solid rgba(255,45,120,0.30)', borderRadius: '8px', fontSize: '12px', fontWeight: 700, color: 'var(--lt-pink-text)' }}>
                                Declined
                              </div>
                            )}
                            {app.status === 'closed' && (
