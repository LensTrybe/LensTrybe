...
export default function ClientDashboardPage() {
  ...
  const s = {
    ...
    threadName: { fontSize: '13px', fontWeight: 600, color: '#f3f3f7', marginBottom: '2px' },
    threadPreview: { fontSize: '12px', color: '#8a8a99' },
    ...
    bubble: (isClient) => ({ padding: '10px 14px', borderRadius: '18px', maxWidth: '55%', background: isClient ? '#1DB954' : '#16161f', color: isClient ? '#000' : '#f3f3f7', border: isClient ? 'none' : '1px solid rgba(255,255,255,0.08)', fontSize: '14px', lineHeight: 1.5, wordBreak: 'break-word', whiteSpace: 'pre-wrap' }),
    replyBar: { padding: '16px 24px', borderTop: '1px solid rgba(255,255,255,0.06)', display: 'flex', gap: '12px', alignItems: 'flex-end' },
    replyInput: { flex: 1, background: '#16161f', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '24px', padding: '10px 16px', color: '#f3f3f7', fontSize: '14px', fontFamily: 'var(--font-ui)', outline: 'none' },
    ...
    empty: { flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#8a8a99', fontSize: '14px' },
    content: { flex: 1, overflowY: 'auto', padding: '24px' },
    grid: { display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '16px', maxWidth: '1200px' },
    creativeCard: { background: '#16161f', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '12px', padding: '16px', cursor: 'pointer', transition: 'border-color 0.15s', maxWidth: '240px', width: '100%' },
    ...
    creativeType: { fontSize: '12px', color: '#8a8a99', marginBottom: '8px' },
    ...
    jobCard: { background: '#16161f', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '12px', padding: '16px', marginBottom: '12px' },
    jobTitle: { fontSize: '15px', fontWeight: 600, marginBottom: '6px' },
    jobMeta: { fontSize: '12px', color: '#8a8a99' },
    ...
    editInput: { background: '#0f0f16', border: '1px solid #1DB954', borderRadius: '6px', padding: '4px 8px', color: '#fff', fontSize: '14px', fontFamily: 'var(--font-ui)' },
    editIcon: { background: 'none', border: 'none', color: '#8a8a99', cursor: 'pointer', fontSize: '12px', padding: '2px' },
  }
  ...
  return (
    <div style={s.page} className="cd-root">
      <style>{`
        /* Shared components mounted here (the notification bell, broadcast banners and
           cards) are built on the --lt-* tokens, which only exist inside DashboardLayout.
           Without them they fall back to their light defaults and render as white panels
           on this dark page, so define the dark set once on the page root. */
        .cd-root {
          ...
          --lt-glass-shadow: 0 24px 60px -22px rgba(0,0,0,0.55), inset 0 1px 0 rgba(255,255,255,0.22);
          ...
          --lt-modal-shadow: 0 40px 100px -30px rgba(0,0,0,0.75), inset 0 1px 0 rgba(255,255,255,0.22);
      ...
      <div style={s.body} className="cd-body">
        <div style={s.sidebar} className="cd-side">
          ...
          <div className="cd-nav" style={s.navItem(view === 'messages')} onClick={() => setView('messages')}>Messages {threads.length > 0 && `(${threads.length})`}</div>
          <div className="cd-nav" style={s.navItem(view === 'bookings')} onClick={() => setView('bookings')}>My Bookings</div>
          <div className="cd-nav" style={s.navItem(view === 'creatives')} onClick={() => setView('creatives')}>My Creatives {creatives.length > 0 && `(${creatives.length})`}</div>
          <div className="cd-nav" style={s.navItem(view === 'jobs')} onClick={() => setView('jobs')}>My Jobs {jobs.length > 0 && `(${jobs.length})`}</div>
          <div className="cd-nav" style={s.navItem(false)} onClick={() => navigate('/creatives')}>Find a Creative</div>
          <div className="cd-nav" style={s.navItem(false)} onClick={() => navigate('/jobs')}>Job Board</div>
          <div className="cd-nav" style={s.navItem(view === 'account')} onClick={() => setView('account')}>Account &amp; Data</div>
        ...
        <div style={s.main}>
            ...
            selected ? (
              <>
                ...
                <div style={s.messageList}>
                  {messages.length === 0
                    ? <div style={{ color: '#8a8a99', fontSize: '14px', textAlign: 'center', marginTop: '40px' }}>No messages yet.</div>
                    : messages.map(m => (
          ...
          {view === 'creatives' && (
            <div style={s.content}>
              ...
              {creatives.length === 0 ? (
                <div style={{ color: '#8a8a99', fontSize: '14px' }}>
                ...
                <div style={s.grid}>
                  {creatives.map(c => (
                    ...
                    >
                      ...
                      {c.city && <div style={{ fontSize: '12px', color: '#8a8a99', marginBottom: '12px' }}>{c.city}{c.state ? `, ${c.state}` : ''}</div>}
                      <div style={{ display: 'flex', gap: '8px' }}>
                        <button style={s.messageBtn} onClick={() => goToCreativeThread(c.id)}>Message</button>
                        <button style={{ ...s.messageBtn, background: 'transparent', color: '#b0b0be' }} onClick={() => navigate(`/creatives/${c.id}`)}>View Profile</button>
                      </div>
                    </div>
                  ))}
                </div>
          ...
          {view === 'account' && (
            <div style={s.content}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', maxWidth: '640px' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', padding: '24px', borderRadius: '18px', background: '#16161f', border: '1px solid rgba(239,68,68,0.3)' }}>
                  <div style={{ fontSize: '15px', fontWeight: 800, color: '#ef4444' }}>Delete account</div>
                  <div style={{ fontSize: '13px', lineHeight: 1.6, color: '#8a8a99' }}>Close your LensTrybe account. Your account, messages and saved creatives are permanently deleted after 30 days, and you can reactivate any time before then by signing in. We'll email you a code to confirm it's you.</div>
          ...
          {view === 'jobs' && (
            <div style={s.content}>
              ...
              {jobs.length === 0 ? (
                <div style={{ color: '#8a8a99', fontSize: '14px' }}>You haven&apos;t posted any jobs yet.</div>
              ) : jobs.map(job => (
                <div key={job.id} style={{ background: '#16161f', border: '1px solid rgba(255,255,255,0.10)', borderRadius: '12px', marginBottom: '12px', overflow: 'hidden' }}>
                  <div
                    style={{ padding: '16px 20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', cursor: 'pointer' }}
                    onClick={() => setExpandedJob(expandedJob === job.id ? null : job.id)}
                  >
                    <div>
                      <div style={{ fontSize: '15px', fontWeight: 600, color: '#f3f3f7' }}>{job.title}</div>
                      <div style={{ fontSize: '12px', color: '#8a8a99', marginTop: '4px' }}>{job.location} · {job.budget_range}</div>
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                      ...
                      <span style={{ color: '#8a8a99', fontSize: '16px' }}>{expandedJob === job.id ? '▲' : '▼'}</span>
                    </div>
                  </div>
                  ...
                  {expandedJob === job.id && (
                    <div style={{ borderTop: '1px solid rgba(255,255,255,0.06)', padding: '16px 20px' }}>
                      {!job.job_applications?.length ? (
                        <div style={{ fontSize: '13px', color: '#8a8a99', padding: '16px 0' }}>No applications yet.</div>
                      ) : (
                        job.job_applications.map(app => (
                          <div key={app.id} style={{ padding: '16px', background: '#0f0f16', borderRadius: '10px', marginBottom: '10px', border: '1px solid rgba(255,255,255,0.10)' }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '10px' }}>
                              <div>
                                <div style={{ fontSize: '14px', fontWeight: 700, color: '#f3f3f7' }}>{app.creative_name}</div>
                                <div style={{ fontSize: '12px', color: '#8a8a99', marginTop: '2px' }}>Applied {new Date(app.created_at).toLocaleDateString('en-AU', { day: 'numeric', month: 'short' })}</div>
                              </div>
                              <div style={{ fontSize: '18px', fontWeight: 800, color: '#1DB954' }}>AUD {Number(app.price ?? 0).toFixed(2)}</div>
                            </div>
                            {app.includes && (
                              <div style={{ marginBottom: '8px' }}>
                                <div style={{ fontSize: '11px', fontWeight: 700, color: '#8a8a99', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: '4px' }}>What&apos;s included</div>
                                <div style={{ fontSize: '13px', color: '#b0b0be', lineHeight: 1.5 }}>{app.includes}</div>
                              </div>
                            )}
                            {app.description && (
                              <div>
                                <div style={{ fontSize: '11px', fontWeight: 700, color: '#8a8a99', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: '4px' }}>Cover message</div>
                                <div style={{ fontSize: '13px', color: '#b0b0be', lineHeight: 1.5 }}>{app.description}</div>
                              </div>
                            )}
                            ...
                            {app.status === 'closed' && (
                              <div style={{ marginTop: '12px', padding: '8px 12px', background: '#0f0f16', border: '1px solid rgba(255,255,255,0.10)', borderRadius: '8px', fontSize: '12px', color: '#8a8a99' }}>
                                Position filled
                              </div>
                            )}
                          </div>
