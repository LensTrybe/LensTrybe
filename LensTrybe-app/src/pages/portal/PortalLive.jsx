      ...
      <main className="pwrap">
        ...
        <div className="pthread2">
          ...
          {items.map((it, n) => {
            ...
            if (it.k === 'meeting') { const m = it.m, s = low(m.status); return <div key={m.id} className="pcard lg"><i className="pk cal"><Icon name="chat" size={13} /></i><div><b>{m.title || 'Meeting'}{m.meeting_date ? ' · ' + nice(m.meeting_date) : ''}</b><small>{[fmtTime(m.start_time), m.meeting_type, m.location].filter(Boolean).join(' · ')}</small></div><span className={'st ' + (s === 'confirmed' ? 'ok' : s === 'declined' ? 'pink' : 'live')}>{s === 'confirmed' ? 'Confirmed' : s === 'declined' ? 'Declined' : s === 'reschedule' ? 'Time suggested' : 'Proposed'}</span>{s !== 'confirmed' && s !== 'declined' && m.response_token && <a className="pbtn" href={'/meeting/' + m.response_token}>Respond</a>}</div> }
            return null
          })}
