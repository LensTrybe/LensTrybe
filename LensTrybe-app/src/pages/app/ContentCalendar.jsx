...
export default function ContentCalendar() {
  ...
  const [start, setStart] = useState(monday(TODAY)), [sel, setSel] = useState(3), [drag, setDrag] = useState(null), [over, setOver] = useState(null)
  ...
  const on = id => CHN.find(c => c.id === id)?.on
  ...
  const rewrite = () => { const takes = [p.body, p.body.split('. ')[0] + '. That is the whole story.', 'Quick one: ' + p.body.charAt(0).toLowerCase() + p.body.slice(1)]; F.open({ title: 'Rewrite', sub: 'Three takes in your voice, from your last forty posts. Pick one or edit it.', cta: 'Use this', fields: [{ k: 'body', l: 'Caption', type: 'select', value: takes[0], options: takes.map((t, i) => [t, ['As written', 'Shorter', 'Casual'][i] + ' · ' + t.slice(0, 48) + '…']) }, { k: 'edit', l: 'Or edit', type: 'textarea', rows: 4, value: p.body }], submit: v => { upd('body', v.edit !== p.body ? v.edit : v.body); toast('Caption updated.') } }) }
      ...
      <div className="grid">
        ...
        {p ? <div className="s4 side">
          <div className="card lg gsel">
            ...
            <div className="ctas">
              {p.st === 'posted' ? <Link className="btn g sm" to="/app/performance">Performance</Link>
                : p.st === 'scheduled' || p.st === 'today' ? <><button className="btn w sm" onClick={() => F.publishPost(p.id)}>Post now</button><button className="btn g sm" onClick={() => F.unschedulePost(p.id)}>Unschedule</button><button className="btn g sm" onClick={movePost}>Move</button></>
                : <><button className="btn w sm" onClick={() => F.schedulePost(p.id)}>Schedule <Icon name="arrow" size={13} /></button><button className="btn g sm" onClick={() => F.publishPost(p.id)}>Post now</button><button className="btn g sm" onClick={movePost}>Move</button></>}
