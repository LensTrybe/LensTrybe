...
export default function Clients({ kind = 'clients' }) {
  const F = useFlows(); const { s, nav, toast } = F; const PEOPLE = s.people, THREADS = s.threads; const uid = useAuth().user?.id
  ...
  const dropTo = (id, name, stages) => { if (LIVE) return toast('The stage moves by itself: send a quote, a contract or an invoice from the thread and the card follows.'); const t = THREADS.find(x => x.id === id); if (!t || stages.includes(t.stage)) return; F.upd('threads', id, { stage: COLSTAGE[name] }); const pj = s.projects.find(x => x.t === id); if (pj) { const map = { Enquiry: 'enq', Quoted: 'quote', Booked: 'booked', Delivered: 'deliv' }; if (s.stages.find(x => x.id === map[name])) F.upd('projects', pj.id, { stage: map[name], k: name === 'Delivered' ? 'done' : 'live' }) } toast(t.n + ' moved to ' + name + '.') }
  const list = useMemo(() => PEOPLE.filter(p => (k === 'All' || p.kind === k || (p.tags || []).includes(k)) && (!q || (p.n + p.co + p.t + p.em + (p.tags || []).join(' ')).toLowerCase().includes(q.toLowerCase()))), [PEOPLE, q, k])
  ...
  // live: every change is a crm_contacts write (a thread-only person gets saved on first edit)
  const saveLive = async (patch, msg) => { try { await live.saveContact(uid, { n: c.n, co: c.co, em: c.em, ph: c.ph, ...patch }, c.crm); await F.refreshLive(); toast(msg || 'Saved.') } catch (e) { toast(e.message); return false } }
  const editLive = () => F.open({ title: c.n, cta: 'Save', working: 'Saving', fields: [{ k: 'n', l: 'Name', required: true, value: c.n }, { k: 'co', l: 'Company or contact', half: true, value: c.co !== c.n ? c.co : '' }, { k: 'status', l: 'Status', half: true, type: 'select', value: c.crm?.status || 'Lead', options: ['Lead', 'In Discussion', 'Booked', 'Completed', 'Archived'] }, { k: 'em', l: 'Email', half: true, type: 'email', value: c.em }, { k: 'ph', l: 'Phone', half: true, type: 'tel', value: c.ph }, { k: 'ig', l: 'Instagram', half: true, value: c.social?.ig || '' }, { k: 'web', l: 'Website', half: true, value: c.social?.web || '' }], alt: c.crm ? { l: 'Remove contact', on: () => { F.confirm({ title: 'Remove ' + c.n + '?', body: 'Threads, quotes and invoices stay. Only the saved contact goes.', cta: 'Remove', danger: true, onYes: async () => { try { await live.deleteContact(c.crm.id); await F.refreshLive(); setSel(null); toast(c.n + ' removed.') } catch (e) { toast(e.message) } } }); return false } } : undefined, submit: v => saveLive(v, c.crm ? 'Saved.' : c.n + ' saved to your contacts.') })
  const editClient = () => LIVE ? editLive() : F.open({ title: c.n, cta: 'Save', fields: [{ k: 'n', l: 'Name', required: true, value: c.n }, { k: 'co', l: 'Contact', half: true, value: c.co }, { k: 'kind', l: 'Kind', half: true, type: 'select', value: c.kind, options: KINDS.slice(1).concat(['Elopement', 'Family', 'Other']) }, { k: 'em', l: 'Email', half: true, type: 'email', value: c.em }, { k: 'ph', l: 'Phone', half: true, type: 'tel', value: c.ph }, { k: 't', l: 'One line', value: c.t }, ...F.socialFields(c.social || {})], alt: { l: 'Remove client', on: () => { F.confirm({ title: 'Remove ' + c.n + '?', body: 'The thread and ledger stay. Only the contact card goes.', cta: 'Remove', danger: true, onYes: () => { F.del('people', c.id); setSel(null); toast(c.n + ' removed.') } }); return false } }, submit: v => { const { n, co, kind, em, ph, t } = v; F.upd('people', c.id, { n, co, kind, em, ph, t, social: F.socialOf(v) }); if (THREADS.find(t => t.id === c.id)) F.upd('threads', c.id, { n }); toast('Saved.') } })
  const addTag = () => F.open({ title: 'Tag ' + c.n, cta: 'Add tag', fields: [{ k: 'tag', l: 'Tag', required: true, placeholder: 'Referral: Ana' }], submit: v => { if (LIVE) return saveLive({ tags: [...(c.tags || []), v.tag] }, 'Tagged.'); F.upd('people', c.id, p => ({ tags: [...p.tags, v.tag] })); toast('Tagged.') } })
  const addNote = () => F.open({ title: 'Note on ' + c.n, cta: 'Save', fields: [{ k: 'notes', l: 'Notes', type: 'textarea', rows: 6, value: c.notes }], submit: v => { if (LIVE) return saveLive({ notes: v.notes }); F.upd('people', c.id, { notes: v.notes }); toast('Saved.') } })
  const importCsv = () => F.open({ title: 'Import contacts', sub: 'Paste names and emails, one per line. Name, email, phone.', cta: 'Import', fields: [{ k: 'csv', l: 'Contacts', type: 'textarea', rows: 8, required: true, placeholder: 'Harper Ellis, harper@gmail.com, 0412 000 000' }, { k: 'kind', l: 'Tag them as', type: 'select', value: 'Wedding', options: KINDS.slice(1) }], submit: async v => { if (LIVE) { const rows = v.csv.split('\n').map(l => l.split(',').map(x => x.trim())).filter(l => l[0]); try { const n = await live.importContacts(uid, rows, v.kind); await F.refreshLive(); toast(n + ' contact' + (n === 1 ? '' : 's') + ' imported' + (n < rows.length ? '. Your plan limit stopped the rest.' : '.')) } catch (e) { toast(e.message); return false } return } let n = 0; v.csv.split('\n').map(l => l.split(',').map(x => x.trim())).filter(l => l[0]).forEach(([name, em, ph]) => { F.ensure({ client: '__new', newName: name, newEmail: em, newPhone: ph }, v.kind + ' · imported', 'TBC', v.kind); n++ }); toast(n + ' contact' + (n === 1 ? '' : 's') + ' imported.') } })
  return (
      ...
      <div className="grid">
        ...
        {c && <div className="s4 side">
          <div className="card lg who">
            ...
            <div className="tags" style={{ marginTop: 10 }}>{(c.tags || []).map(t => <span key={t} title="Remove" style={{ cursor: 'pointer' }} onClick={() => LIVE ? saveLive({ tags: c.tags.filter(x => x !== t) }, 'Tag removed.') : F.upd('people', c.id, p => ({ tags: p.tags.filter(x => x !== t) }))}>{t}</span>)}<button className="tag-add" onClick={addTag}>+</button></div>
            <div className="ctas" style={{ marginTop: 14 }}>
              {THREADS.find(t => t.id === c.id) ? <Link className="btn w sm" to={'/app/thread/' + c.id}>Open thread <Icon name="arrow" size={13} /></Link> : <button className="btn w sm" onClick={() => { if (LIVE) { if (!c.em) return toast('Add their email first, then start a thread.'); return F.newThread({ name: c.n, email: c.em }) } F.ensure({ client: c.id }); nav('/app/thread/' + c.id) }}>New thread</button>}
              {LIVE && !c.crm && <button className="btn g sm" onClick={() => saveLive({}, c.n + ' saved to your contacts.')}>Save contact</button>}
              <button className="btn g sm" onClick={() => F.newDoc('q', LIVE ? { client: c.id } : { client: c.id, d: c.jobs[0]?.[0] || '', v: c.jobs[0]?.[2] || '' })}>Quote</button>
          ...
          <div className="card lg"><div className="h"><b>Notes</b>{!LIVE && <small className="lumi-by"><span className="lm" style={{ width: 12, height: 12 }} />Lumi keeps these</small>}</div><p className="note2">{c.notes || 'Nothing yet.'}</p><button className="lnk" style={{ marginTop: 10 }} onClick={addNote}>{c.notes ? 'Edit' : 'Add a note'}</button></div>
        </div>}
      </div>
    </section>
  )
