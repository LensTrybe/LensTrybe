...
export default function DocEditor({ kind = 'inv' }) {
  ...
  const blank = () => ({
    ...
    dep: { on: isQ ? true : !!D.depOn, pct: D.depPct ?? 30, amt: '', due: TODAY }, balDue: addDays(TODAY, isQ ? (D.validDays ?? 9) : (D.balDays ?? 7)),
  ...
  const save = (st = status === 'Draft' ? 'Draft' : status) => { if (existing) F.upd('ledger', existing.id, record(st)); else { F.add('ledger', { id: inv.no, ...record(st) }, kind) } setDirty(false); return inv.no }
  const send = () => {
  ...
  const del = () => F.confirm({ title: 'Delete ' + inv.no + '?', body: 'It comes out of the ledger and the thread.', cta: 'Delete', danger: true, onYes: () => { if (existing) F.removeDoc(existing.id); nav(base) } })
  const accept = () => { save('Accepted'); F.markAccepted(existing.id); nav(base) }
  ...
  return (
    <section className="view inved">
      <div className="vh">
        ...
        <div className="acts">
          {existing && !isQ && status !== 'Paid' && <button className="btn g" onClick={() => { F.markPaid(existing.id); nav(base) }}><Icon name="check" size={15} />Mark paid</button>}{existing && isQ && status !== 'Accepted' && <button className="btn g" onClick={accept}><Icon name="check" size={15} />Mark accepted</button>}{existing && isQ && status === 'Accepted' && <button className="btn g" onClick={() => F.invoiceFromQuote(existing.id)}><Icon name="dollar" size={15} />Invoice</button>}
          ...
          <button className="btn w" onClick={send}><Icon name="arrow" size={15} />{status === 'Sent' ? 'Send again' : 'Send ' + LABEL.toLowerCase()}</button>
        </div>
      </div>
      <div className="grid">
        <div className="s8">
