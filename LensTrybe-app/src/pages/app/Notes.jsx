    ...
    <section className="view fill">
      <div className="tw notes">
        <aside className="tlist lg">
          ...
          <div className="trows">
            {list.map(x => <button key={x.id} type="button" className={'tli' + (n?.id === x.id ? ' on' : '')} onClick={() => setSel(x.id)}><span className="nic"><Icon name="note" size={15} /></span><div className="tx"><div className="r1"><b>{x.t}{x.pinned && <i className="pinmark">★</i>}</b><small>{x.w}</small></div><span className="r2">{(x.body || '').split('\n')[0] || 'Empty'}</span>{who(x) && <span className="r3"><i>{who(x)}</i></span>}</div></button>)}
        ...
        {n ? <div className="tp lg npane">
          <div className="tph">
            ...
            <div className="acts">{LIVE && <button className={'ic2' + (n.pinned ? ' on' : '')} title={n.pinned ? 'Unpin' : 'Pin to the top'} aria-label="Pin" onClick={() => { upd('notes', n.id, { pinned: !n.pinned }); toast(n.pinned ? 'Unpinned.' : 'Pinned to the top.') }}><Icon name="star" size={15} /></button>}<button className="ic2" title="Attach to a project or client" aria-label="Attach" onClick={attach}><Icon name="user" size={15} /></button><button className="ic2" title="Delete" aria-label="Delete" onClick={remove}><Icon name="x" size={15} /></button></div>
          </div>
