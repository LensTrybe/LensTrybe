...
import { uuid, isDoneStage } from '../../lib/projsync'
...
export default function ProjectDetail() {
  ...
  const stamp = () => LIVE ? new Date().toLocaleDateString('en-AU', { day: 'numeric', month: 'short' }) : 'Just now'
  const log = txt => upd('projects', p.id, x => ({ log: [[stamp(), txt], ...(x.log || [])] }))
  ...
  const addTask = () => { const t = newTask.trim(); if (!t) return; P(x => ({ tasks: [...(x.tasks || []), LIVE ? [t, 0, '', uuid()] : [t, 0, '']] })); setNewTask('') }
  const taskDue = i => F.open({ title: 'Task', cta: 'Save', fields: [{ k: 't', l: 'Task', required: true, value: tasks[i][0] }, { k: 'd', l: 'Due', type: 'date', value: tasks[i][2] || '' }], alt: { l: 'Remove', on: () => P(x => ({ tasks: x.tasks.filter((_, j) => j !== i) })) }, submit: v => P(x => ({ tasks: x.tasks.map((t, j) => j === i ? [v.t, t[1], v.d, t[3]] : t) })) })
  const tickTask = i => P(x => ({ tasks: x.tasks.map((t, j) => j === i ? [t[0], t[1] ? 0 : 1, t[2], t[3]] : t) }))
  const addList = () => F.open({ title: 'New checklist', cta: 'Add', fields: [{ k: 'n', l: 'Name', required: true, placeholder: 'Shot list' }, { k: 'tpl', l: 'Start from', type: 'select', value: '', options: [['', 'Empty'], ...s.checklistTemplates.map(t => [t.id, t.n])] }], submit: v => { const tpl = s.checklistTemplates.find(t => t.id === v.tpl); P(x => ({ lists: [...(x.lists || []), { id: LIVE ? uuid() : 'l' + Date.now().toString(36), n: v.n, items: tpl ? tpl.items.map(i => LIVE ? [i, 0, uuid()] : [i, 0]) : [] }] })); toast('Checklist added.') } })
  ...
  const saveTpl = l => { F.add('checklistTemplates', { id: LIVE ? uuid() : 'tp' + Date.now().toString(36), n: l.n, items: l.items.map(i => i[0]) }, 'any', false); toast('"' + l.n + '" saved as a template. It is on every new project.') }
  const tickItem = (lid, i) => P(x => ({ lists: x.lists.map(l => l.id === lid ? { ...l, items: l.items.map((it, j) => j === i ? (it.length > 2 ? [it[0], it[1] ? 0 : 1, it[2]] : [it[0], it[1] ? 0 : 1]) : it) } : l) }))
  const addItem = lid => { const t = (newItem[lid] || '').trim(); if (!t) return; P(x => ({ lists: x.lists.map(l => l.id === lid ? { ...l, items: [...l.items, LIVE ? [t, 0, uuid()] : [t, 0]] } : l) })); setNewItem(o => ({ ...o, [lid]: '' })) }
  ...
  const finish = () => F.confirm({ title: 'Mark ' + p.n + ' delivered?', body: 'Moves to Delivered, the thread moves to Deliver, and Lumi asks for a review in three days.', cta: 'Delivered', onYes: () => { F.setProjectStage(p.id, (LIVE ? s.stages.find(x => /deliver/i.test(x.n)) || s.stages.find(isDoneStage) : s.stages.find(x => x.id === 'deliv'))?.id || s.stages[s.stages.length - 1].id); if (p.t) F.upd('threads', p.t, { next: 'Delivered · ask for a review' }) } })
