...
const KINDS0 = [['all', 'All'], ['gallery', 'From galleries'], ['review', 'From reviews'], ['gap', 'Fill a gap'], ['behind', 'Behind the scenes'], ['season', 'Seasonal']]
const KINDS = LIVE ? [['all', 'All']] : KINDS0
...
export default function ContentIdeas() {
  ...
  const keep = id => { const x = ideas.find(y => y.id === id); F.add('posts', { ...(LIVE ? { id: uuid() } : {}), d: x.bd || addDays(TODAY, 2), ch: x.ch, t: x.t, body: x.cap, st: 'draft', s: x.s, m: x.m, stats: '', time: '19:30' }, 'post'); F.del('ideas', id); nextSel(id); toast(LIVE ? 'Added to the calendar as a draft for ' + (x.bd || addDays(TODAY, 2)).split('-').reverse().slice(0, 2).join('/') + '.' : 'Added to the calendar as a draft for ' + (x.best || 'this week') + '.') }
  const pass = id => { if (LIVE) { F.del('ideas', id); nextSel(id); return toast('Removed.') } const x = ideas.find(y => y.id === id); F.add('passed', { from: 'Lumi', j: x.t, w: x.k, g: '' }, 'any'); F.del('ideas', id); nextSel(id); toast('Passed. Lumi will suggest fewer like it.') }
  ...
  return (
    <section className="view">
      <div className="vh">
        <div><h1>Content ideas</h1><p>{LIVE ? 'Jot ideas down as they come. Keep one and it lands on the calendar as a draft.' : 'Drafted from work you\'ve already done. Keep the good ones, they land on the calendar as drafts.'}</p></div>
        <div className="acts"><Link className="btn g" to="/app/content-calendar"><Icon name="cal" size={15} />Calendar</Link>{LIVE ? <button className="btn w" onClick={F.newIdea}><Icon name="plus" size={15} />New idea</button> : <button className="btn w" onClick={more}><span className="lm" style={{ width: 14, height: 14 }} />More like this</button>}</div>
      </div>
      <div className="grid">
        <div className="s12"><div className="kp">
          {(LIVE ? [['Ideas waiting', ideas.length, 'yours, ready to plan', ''], ['On the calendar', String((s.posts || []).filter(p => p.st !== 'posted').length), 'drafts and planned posts', ''], ['Posted this month', String((s.posts || []).filter(p => p.st === 'posted' && p.d.startsWith(TODAY.slice(0, 7))).length), 'marked as posted', 'n']] : [['Ideas waiting', ideas.length, 'refreshed this morning', ''], ['Kept this month', '11', '8 posted, 3 drafts', ''], ['Best day to post', 'Thursday', '7:30 pm, from your last 90 days', 'n'], ['Posts that brought enquiries', '3', 'all from galleries', 'n']]).map(([l, v, e, w]) => <div key={l} className="k lg"><small>{l}</small><b>{v}</b><em className={w}>{e}</em></div>)}
        </div></div>
        ...
        <div className="card lg s8">
          ...
          <div className="ideas">
            ...
            {!list.length && <div className="tempty">{LIVE ? <>No ideas yet. <button className="lnk" onClick={F.newIdea}>Add one</button></> : <>Nothing left here. Lumi adds more as work is delivered. <button className="lnk" onClick={more}>Ask for more</button></>}</div>}
          </div>
        </div>

        {i ? <div className="s4 side">
          <div className="card lg gsel">
            <div className="cover"><Still seed={i.s + 40} mood={i.m} /><div className="in"><b>{i.t}</b><small>{i.best ? 'Best time · ' + i.best : 'Idea'}</small></div></div>
          ...
          {LIVE ? (i.why ? <div className="card lg"><div className="h"><b>Why now</b></div><p className="note2">{i.why}</p></div> : null) : <div className="card lg"><div className="h"><b>Why this one</b><small className="lumi-by"><span className="lm" style={{ width: 12, height: 12 }} />Lumi</small></div><p className="note2">{i.why} Captions are in your voice from the last forty posts. Nothing goes out until you schedule it.</p></div>}
