    ...
    <main className="page" style={{ paddingTop: 96 }}><div className="wrap">
      ...
      <div className="plyt">
        <div>
          ...
          {tab === 2 && <div className="revs">{c.reviews.length ? c.reviews.map(r => <div key={r.id} className="rc lg"><div className="h"><b>{r.who}</b><i>{'★'.repeat(r.r)}{'☆'.repeat(5 - r.r)}</i></div><p>{r.text}</p><small>{[r.kind, r.when ? new Date(r.when).toLocaleDateString('en-AU', { month: 'long', year: 'numeric' }) : null, r.verified ? 'Verified booking' : 'Imported'].filter(Boolean).join(' · ')}</small></div>) : <p className="fine">No reviews yet. The first one comes from the first job booked here.</p>}</div>}
