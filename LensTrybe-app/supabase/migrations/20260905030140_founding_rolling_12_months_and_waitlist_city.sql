-- 1) Waitlist: add city for Brisbane focus + city-level expansion heatmap
alter table public.waitlist add column if not exists city text;

-- 2) Founding grant: rolling 12 months free Expert from each creative's own signup
--    (was a fixed date of 2027-10-01). Only this one line changes vs the live function.
