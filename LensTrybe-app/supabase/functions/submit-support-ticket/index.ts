...
Deno.serve(async (req) => {
  ...
  if (insErr) return json({ error: 'Could not submit your request. Please try again.' }, 500)
  ...
  await sendEmail(resendKey, {
    ...
    html: emailShell({
      ...
      heading: subject || 'New support request',
  ...
  await sendEmail(resendKey, {
    ...
    html: emailShell({
      ...
      heading: `Thanks${name ? `, ${esc(name)}` : ''}. We're on it.`,
      ...
      panelHtml: panel(
        (subject ? fieldRow('Subject', esc(subject)) : '') +
        (category ? fieldRow('Category', esc(category)) : '') +
        `<div style="font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:0.06em;color:${BRAND.faint};margin-bottom:3px;">Your message</div><div style="font-size:14px;color:${BRAND.text};line-height:1.7;">${nl2br(message)}</div>`
      ),
      footNote: 'You can reply straight to this email if you need to add anything.',
    }),
  })
