...
serve(async (req) => {
  ...
  try {
    ...
    // Only the signed-in creative who owns the quote can send it.
    ...
    const quoteId = String(body?.quote_id ?? body?.quoteId ?? body?.quote?.id ?? '')
    if (!UUID_RE.test(quoteId)) return jsonRes({ error: 'quote_id required' }, 400)

    const { data: record, error: loadErr } = await supabase.from('quotes').select('*').eq('id', quoteId).maybeSingle()
    if (loadErr) console.error('send-quote load failed', loadErr)
    if (!record || record.creative_id !== user.id) return jsonRes({ error: 'Quote not found' }, 404)
    if (!isEmail(record.client_email)) return jsonRes({ error: 'Add a valid client email to this quote before sending.' }, 400)

    const allowed = await supabase.rpc('rate_limit_hit', { p_key: 'send-quote:' + user.id, p_max: 60, p_window_seconds: 3600 })
    if (allowed.error) console.error('send-quote rate limit check failed', allowed.error)
    else if (allowed.data === false) return jsonRes({ error: 'Too many quotes sent recently. Please try again later.' }, 429)
    ...
    const quote: any = { ...record, line_items: Array.isArray(record.line_items) ? record.line_items : (Array.isArray(record.items) ? record.items : []) }
...
<tr><td style="padding:28px 36px 32px;"><div style="border-top:1px solid rgba(255,255,255,0.08);padding-top:18px;"><div style="font-size:12px;font-weight:400;letter-spacing:0.24em;color:#ffffff;">LENSTRYBE</div><div style="font-size:12px;color:#6a6a78;margin-top:2px;">Connect. Capture. Create.</div></div></td></tr>
    ...
    if (!res.ok) {
      ...
      if (res.status === 429 || /quota/i.test(JSON.stringify(data ?? ''))) {
        console.error('send-quote resend DAILY QUOTA exhausted', res.status, data)
        return jsonRes({ error: 'Our email service has hit its daily sending limit, so this was not sent. That is a problem on our end, not with your quote. Sending will work again once the limit resets.' }, 503)
      }
