...
serve(async (req) => {
  ...
  try {
    const { invoice, profile, bankDetails } = await req.json()
...
<body style="font-family:Arial,sans-serif;max-width:680px;margin:0 auto;padding:48px;color:#111">
  <table width="100%" style="margin-bottom:40px">
    <tr>
      ...
      <td style="text-align:right">
        <div style="font-size:30px;font-weight:800;color:#1DB954">INVOICE</div>
        <div style="font-size:13px;color:#666">#${invoice.id.slice(0, 8).toUpperCase()}</div>
        <div style="font-size:13px;color:#666">${new Date(invoice.created_at).toLocaleDateString('en-AU', { day: 'numeric', month: 'long', year: 'numeric' })}</div>
        ${invoice.due_date ? `<div style="font-size:13px;color:#666">Due: ${new Date(invoice.due_date).toLocaleDateString('en-AU', { day: 'numeric', month: 'long', year: 'numeric' })}</div>` : ''}
      </td>
    </tr>
  </table>
  ...
  <div style="margin-bottom:32px">
    ...
    <div style="font-size:15px;font-weight:600;color:#111">${invoice.client_name}</div>
    <div style="font-size:13px;color:#666">${invoice.client_email}</div>
    ${invoice.client_phone ? `<div style="font-size:13px;color:#666">${invoice.client_phone}</div>` : ''}
    ${invoice.client_address ? `<div style="font-size:13px;color:#666">${invoice.client_address}</div>` : ''}
  </div>
      ...
      <td width="240" style="border-top:2px solid #111;padding-top:8px">
        <table width="100%">
          <tr>
            ...
            <td style="font-size:15px;font-weight:800;color:#111;text-align:right">AUD ${Number(invoice.amount).toFixed(2)}</td>
          </tr>
        </table>
      </td>
    </tr>
  ...
  <div style="margin-top:40px;padding-top:20px;border-top:1px solid #e5e7eb;font-size:12px;color:#999;text-align:center">
    ...
    // Since we can't use a headless browser in Edge Functions, we send the HTML as an attachment
    // Resend supports sending HTML attachments — client can open and print/save as PDF
    ...
    const emailBody = `
      <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;background:#080810;color:#fff;padding:40px 32px;border-radius:12px">
        <div style="margin-bottom:24px">
          <span style="font-size:22px;font-weight:700;color:#1DB954">LensTrybe</span>
        </div>
        <h2 style="font-size:20px;font-weight:600;color:#fff;margin:0 0 8px">You have a new invoice</h2>
        <p style="color:#888;font-size:14px;margin:0 0 24px">
          <strong style="color:#fff">${profile?.business_name ?? 'Your creative'}</strong> has sent you an invoice for <strong style="color:#fff">AUD ${Number(invoice.amount).toFixed(2)}</strong>.
        </p>
        <p style="color:#888;font-size:13px;margin:0 0 8px">The invoice is attached to this email as an HTML file. Open it in your browser and press <strong style="color:#fff">Ctrl+P</strong> (or Cmd+P on Mac) then select <strong style="color:#fff">Save as PDF</strong> to download it.</p>
    ...
    const res = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        ...
        subject: `Invoice from ${profile?.business_name ?? 'Your Creative'} — AUD ${Number(invoice.amount).toFixed(2)}`,
        html: emailBody,
        attachments: [
          {
