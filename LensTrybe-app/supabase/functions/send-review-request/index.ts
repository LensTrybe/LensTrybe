...
Deno.serve(async (req) => {
  ...
  const notePanel = message
    ? panel(`<p style="margin:0 0 10px;color:${BRAND.faint};font-size:12px;font-weight:700;text-transform:uppercase;letter-spacing:0.06em;">A note from ${esc(businessName)}</p><p style="margin:0;color:${BRAND.text};font-size:15px;line-height:1.7;">${nl2br(message)}</p>`)
    : panel(`<p style="margin:0;color:${BRAND.muted};font-size:14px;line-height:1.7;">It only takes a minute, and it helps ${esc(businessName)} reach more clients on LensTrybe.</p>`)
  ...
  const res = await sendEmail(resendKey, {
    ...
    html: emailShell({
      ...
      heading: `${greeting.replace(/,$/, '')} how was working with ${esc(businessName)}?`,
      intro: `${esc(businessName)} would love a quick review of your experience. Your words help other clients know what to expect.`,
      panelHtml: notePanel,
      ctaText: 'Leave a review',
      ctaUrl: reviewUrl,
      footNote: `Or paste this link into your browser: ${reviewUrl}`,
    }),
  })
