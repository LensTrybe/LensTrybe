...
Deno.serve(async (req) => {
  ...
  const panelHtml = panel(
    ...
    fieldRow('Signed by', `${esc(clientName)}${contract.client_email ? ` &middot; ${esc(contract.client_email)}` : ''}`) +
    fieldRow('Signed at', esc(signedAt))
  )
  ...
  await sendEmail(resendKey, {
    ...
    html: emailShell({
      ...
      heading: `${esc(clientName)} signed your contract`,
